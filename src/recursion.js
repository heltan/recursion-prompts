...
var reverse = function(string) {
    ...
    return (string.charAt(`${index}`) + reverse(string.substring(0, length -1))) ;
};
...
var reverseArr = function(array) {
    ...
    return [array[array.length-1]].concat(reverseArr(array.slice(0,length-1)));
};
...
var countOccurrence = function(array, value) {
  // the array[0] is a number type, should equal value
...
var rMap = function(array, callback) {
    ...
    return [callback(array[0])].concat(rMap(array = array.slice(1), callback));
...
//var count = 0; //no...this is keeping the same count every time you run the function again
...
var countValuesInObj = function(obj, value) {
    var count = 0;
    for (var keys in obj) {
        if (obj[keys] === value && typeof obj[keys] === 'string') {
            count ++;
        }
        if (typeof obj[keys] === 'object') {
            if (obj[keys] === value) {
...
var fibonacci = function(n) { 
    ...
    //first check if n ===0
    //then for the recursion do a if (n-1)===0 and the array length is one, which means nothing was added to it;
    ...
    if (count <= n) {
        var num = (arr[count-1] + arr[count-2]);
        count++;
        arr.push(num);
        console.log(arr);
